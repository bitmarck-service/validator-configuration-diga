...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="html:document" xmlns="http://www.w3.org/1999/xhtml">
        ...
        <xsl:if test="$input-document instance of document-node(element())">
            <xsl:call-template name="html:contentdoc">
                <xsl:with-param name="document" select="$input-document"/>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template name="html:contentdoc" xmlns="http://www.w3.org/1999/xhtml" >
        <xsl:param name="document" as="document-node(element())"/>
        <p class="important">
            <xsl:text>Inhalt des Dokuments:</xsl:text>
        </p>
        <table class="document">
            <xsl:apply-templates select="$document/*" mode="html:contentdoc"/>
        </table>
    </xsl:template>
